-- ============================================================================
-- 0007 — COMMISSION DE PLATEFORME SUR LES DÉCAISSEMENTS DE TONTINE
-- ============================================================================
-- Jusqu'ici le bénéficiaire d'un cycle encaissait 100 % du pot : aucune
-- commission n'existait nulle part dans le code ni dans le schéma.
--
-- Deux garde-fous volontaires :
--   1. Le taux par défaut est 0 %, donc appliquer cette migration ne change
--      RIEN au comportement actuel. Il faut fixer explicitement le taux et le
--      portefeuille destinataire pour que de l'argent commence à être prélevé.
--   2. La commission passe par le même journal double-entrée que le reste de
--      la comptabilité — jamais un simple UPDATE de solde — pour qu'elle soit
--      auditable ligne par ligne comme les autres mouvements.

ALTER TABLE public.platform_settings
  ADD COLUMN IF NOT EXISTS payout_fee_percent numeric NOT NULL DEFAULT 0
    CHECK (payout_fee_percent >= 0 AND payout_fee_percent <= 100),
  ADD COLUMN IF NOT EXISTS platform_wallet_id uuid REFERENCES public.profiles(id);

COMMENT ON COLUMN public.platform_settings.payout_fee_percent IS
  'Commission prélevée sur le pot à chaque décaissement, en pourcentage. 0 = désactivée.';
COMMENT ON COLUMN public.platform_settings.platform_wallet_id IS
  'Profil dont le portefeuille encaisse la commission. Sans lui, aucune commission n''est prélevée.';
...
BEGIN
  ...
  -- Commission : lue à l'exécution, jamais passée par le client.
  SELECT payout_fee_percent, platform_wallet_id
    INTO v_fee_percent, v_platform_wallet
  FROM public.platform_settings WHERE id = 1;

  IF coalesce(v_fee_percent, 0) > 0 AND v_platform_wallet IS NOT NULL
     AND v_platform_wallet <> p_beneficiary_id THEN
    v_fee := floor(v_total_pot * v_fee_percent / 100);
  END IF;

  v_beneficiary_payout := v_total_pot - p_discount_amount - v_fee;
  -- Un paramétrage aberrant ne doit jamais produire un décaissement négatif
  -- ni vider le pot : on refuse plutôt que de déplacer de l'argent de travers.
  ...
  IF v_fee > 0 THEN
    ...
    INSERT INTO public.double_entry_ledger (id, transaction_id, idempotency_key, account, counterparty, type, amount, currency, description)
    VALUES
      (v_transaction_id || '_fee_dr', v_transaction_id, v_idempotency_key, 'tontine_group:' || p_group_id, 'platform_wallet:' || v_platform_wallet, 'debit', v_fee, v_group.currency, 'Commission plateforme (' || v_fee_percent || ' %) sur le pot de ' || v_total_pot || ' ' || v_group.currency),
      (v_transaction_id || '_fee_cr', v_transaction_id, v_idempotency_key, 'platform_wallet:' || v_platform_wallet, 'tontine_group:' || p_group_id, 'credit', v_fee, v_group.currency, 'Commission plateforme (' || v_fee_percent || ' %) sur le pot de ' || v_total_pot || ' ' || v_group.currency);
  END IF;
